  ...
  const imageSrc = isSpotify
    ...
    : page.data.assets?.large_image
      ? page.data.assets.large_image.startsWith("mp:external")
        ? page.data.assets.large_image.replace(/mp:external\/.*\/https\//, "https://")
    ...
    <div onClick={onClick} className="cursor-pointer">
      <div className="border-2 border-zinc-200 dark:border-zinc-800 p-3 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-right-5 duration-500">
        <Image
          ...
          width={64}
          height={64}
          alt={isSpotify ? `Album cover for ${page.data.song}` : page.data.name || "Activity image"}
        ...
        <div className="flex-1 min-w-0">
          ...
          {isSpotify ? (
            <p className="text-xs text-zinc-600 dark:text-zinc-300">{page.data.artist}</p>
          ) : (
...
export default function DiscordPresence({ userId }: Props) {
  ...
  return (
    <div className="flex flex-col gap-2 w-65 font-sans">
      <ActivityCard page={currentPage} elapsed={elapsed} onClick={handleNextPage} />
